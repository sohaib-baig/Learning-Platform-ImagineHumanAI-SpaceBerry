...
import type { MembershipAuditEntry } from "./auditLogger";
...
interface RequirePaymentOptions {
  ...
  membershipBatchLimit?: number;
  timeoutMs?: number;
  auditLogger?: (entry: MembershipAuditEntry) => Promise<void>;
}
...
export async function enforcePaymentForFreeMembers({
  clubId: rawClubId,
  hostUid,
  db,
  logger = console,
  membershipBatchLimit = DEFAULT_BATCH_LIMIT,
  timeoutMs = DEFAULT_TIMEOUT_MS,
  auditLogger,
}: RequirePaymentOptions): Promise<RequirePaymentResult> {
  ...
  const membershipFieldPath = new FieldPath(
    "clubMemberships",
    clubId,
    "lastPaymentType"
  );
  ...
  while (true) {
    ...
    const batchAuditEntries: MembershipAuditEntry[] = [];

    snap.docs.forEach((docSnap) => {
      const userRef = docSnap.ref;
      const userData = docSnap.data() || {};
      const existingMembership =
        userData.clubMemberships?.[clubId]?.status ?? null;
    ...
    const hasDotKey = snap.docs.some(() =>
      Object.keys({
    ...
    if (hasDotKey) {
      logger.error?.(
        "[RequirePayment] Aborted commit due to dotted field key",
        {
          clubId,
      ...
      throw new Error(
        `[RequirePayment] Invalid dotted key in user ${snap.docs[0].id}`
      );
    }
  ...
  const clubUpdatePayload: firestore.UpdateData<firestore.DocumentData> = {
    updatedAt: FieldValue.serverTimestamp(),
    "info.priceChangedAt": FieldValue.serverTimestamp(),
  };

  if (!alreadyLocked) {
    (clubUpdatePayload as firestore.DocumentData).pricingLocked = true;
  }
